...
doc_events = {
	...
	"Sales Invoice": {
		"on_submit": [
			...
			"erpnext.hooks.on_submit_sales_invoice",
		],
	...
	"Payment Entry": {
		"on_submit": [
			...
			"erpnext.hooks.on_submit_payment_entry",
		],
...
# Create a new file: erpnext/hooks/sip_integration.py with these functions

# For now, add the functions here directly or create the sip_integration module
def on_submit_sales_invoice(doc, method):
	"""Sync Sales Invoice to Laravel SIP when submitted"""
	try:
		import frappe
		import requests
		import json
		...
		paid_amount = float(doc.grand_total) - float(doc.outstanding_amount)
		balance = float(doc.outstanding_amount)
		...
		invoice_data = {
			'erp_invoice_id': doc.name,
			'student_id': student_id,
			'invoice_number': doc.name,
			'invoice_type': doc.get('invoice_type') or 'tuition',
			'total_amount': float(doc.grand_total),
			'paid_amount': paid_amount,
			'balance': balance,
			'status': status,
			'due_date': doc.due_date.strftime('%Y-%m-%d') if doc.due_date else None,
			'issued_date': doc.posting_date.strftime('%Y-%m-%d') if doc.posting_date else None,
		...
		if response.status_code == 200:
			...
			try:
				frappe.db.set_value("Sales Invoice", doc.name, "sip_synced", 1)
			except:
				pass  # Custom field may not exist
			
			frappe.log_error(f"Invoice {doc.name} synced successfully to SIP", "Invoice Sync Success")
		else:
...
def on_submit_payment_entry(doc, method):
	"""Sync Payment Entry to Laravel SIP when submitted"""
	try:
		import frappe
		import requests
		
		# Only process if payment is for a customer (student)
		...
		# Try to get student_id
		student_id = None
		if hasattr(customer, 'student_id') and customer.student_id:
			student_id = customer.student_id
		elif customer.customer_name and 'Student' in customer.customer_name:
			parts = customer.customer_name.split()
			if len(parts) > 1:
				student_id = parts[-1]
		elif customer.name and customer.name.startswith('STU-'):
			student_id = customer.name.replace('STU-', '')
		
		if not student_id:
			return  # Not a student payment, skip
		
		# Get payment reference from linked invoices or use payment entry name
		payment_reference = doc.name
		if doc.references:
			for ref in doc.references:
				if ref.reference_doctype == 'Sales Invoice':
					payment_reference = ref.reference_name
					break
		
		# Prepare payment data
		payment_data = {
			'payment_reference': payment_reference,
			'erp_payment_id': doc.name,
			'status': 'completed' if doc.docstatus == 1 else 'failed',
			'amount': float(doc.paid_amount),
			'payment_date': doc.posting_date.strftime('%Y-%m-%d') if doc.posting_date else None,
			'payment_method': doc.mode_of_payment or '',
		}
		
		# Get Laravel API URL
		laravel_url = frappe.conf.get('laravel_api_url') or 'https://sip.delexesuniversity.edu.gh/api/erp/payments/confirm'
		...
		if response.status_code == 200:
			frappe.log_error(f"Payment {doc.name} synced successfully to SIP", "Payment Sync Success")
		else:
			error_msg = f"Laravel API Error: {response.status_code} - {response.text}"
			frappe.log_error(error_msg, "Payment Sync Error")
			
	except Exception as e:
		error_msg = f"Error syncing payment: {str(e)}"
		frappe.log_error(error_msg, "Payment Sync Error")       
